...
library RedBlackTree {
    ...
    function getFirstId(Tree storage tree) public constant returns (uint64 id) {
        id = tree.root;
        while (id != 0) {
    ...
    function getLastId(Tree storage tree) public constant returns (uint64 id) {
        id = tree.root;
        while (id != 0) {
    ...
    function find(Tree storage tree, uint value) public constant returns (uint64 id) {
        id = tree.root;
        while (id != 0) {
    ...
    function findParent(Tree storage tree, uint value) public constant returns (uint64 parentId) {
        uint64 id = tree.root;
        parentId = id;
        while (id != 0) {
            if (value == tree.items[id].value) {
    ...
    function insert(Tree storage tree, uint64 id, uint value) internal {
        uint64 parent = findParent(tree, value);
    ...
    // BK Ok
    function parent(Tree storage tree, uint64 n) private view returns (uint64) {
